...
test('personal Chrome protocol exposes one typed definition per tool', () => {
  ...
  assert.equal(PERSONAL_CHROME_TOOLS.length, 22);
  ...
  assert.equal(findPersonalChromeTool('browser_workspace')?.scope, 'write');
});
...
test('tool argument validation rejects unknown, malformed, and out-of-range fields', () => {
  ...
  const workspace = findPersonalChromeTool('browser_workspace');
  assert.ok(navigate && key && port && selectionContext && workspace);
  ...
  assert.equal(validatePersonalChromeToolArguments(workspace, { action: 'attach', tabId: 42 }), null);
  assert.match(validatePersonalChromeToolArguments(workspace, { action: 'rename', title: '' }) || '', /too short/);
});
