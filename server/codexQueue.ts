...
export type CodexQueueListItem = Pick<CodexQueueItem,
  | 'id'
  | 'profileId'
  | 'queueKey'
  | 'sessionId'
  ...
  | 'lastRunStatus'
  | 'stopPolicy'
  | 'stopDecisionForItemId'
  | 'continuationOfItemId'
> & {
  detailsIncluded: boolean;
};

export interface CodexQueueWorkspaceList {
  items: CodexQueueListItem[];
...
const MAX_PARALLEL_QUEUE_ITEMS = Math.max(
  1,
  Number(process.env.CODEX_QUEUE_MAX_PARALLEL_ITEMS) || 20
);
const QUEUE_RETENTION_MS = 21 * 24 * 60 * 60 * 1000;
const QUEUE_LIST_RECENT_COMPLETED_LIMIT = 20;
const QUEUE_LIST_RELATED_COMPLETED_LIMIT = 50;
const QUEUE_LIST_ERROR_LIMIT = 2_000;
const STOP_DECISION_PRIORITY = 100;
const STOP_CONTINUATION_PRIORITY = 90;
const MAX_SCHEDULED_STOP_ATTEMPTS = 3;
const QUEUE_SHUTDOWN_TIMEOUT_MS = 8_000;
...
let activeWorkerPromises = new Map<string, Promise<void>>();
...
/**
 * Persist the provider-assigned id as soon as its startup event arrives. The
 * original draft queue key intentionally remains unchanged until the run is
 * finalized, so refreshed clients can still correlate the draft URL with the
 * newly-created session.
 */
export async function bindCodexQueueItemToStartedSession(
  itemId: string,
  sessionId: string
): Promise<CodexQueueItem | null> {
  await ensureStateLoaded();
  const normalizedSessionId = normalizeSessionBindingKey(sessionId);
  ...
  state.sessionBindings[item.queueKey] = normalizedSessionId;
  state.sessionBindings[normalizedSessionId] = normalizedSessionId;
...
function launchQueueItem(item: CodexQueueItem) {
  ...
  const workerPromise = processQueueItem(item).finally(() => {
    ...
    activeWorkerPromises.delete(item.id);
    scheduleImmediateTick();
  });
  activeWorkerPromises.set(item.id, workerPromise);
  void workerPromise;
}
...
async function tickWorker() {
  if (!workerStarted || workerStopping || workerTickInFlight) {
    return;
  }
...
function scheduleImmediateTick() {
  if (QUEUE_EXECUTION_DISABLED || !workerStarted || workerStopping) {
    return;
  }
...
export async function shutdownCodexQueueWorker(): Promise<void> {
  ...
  for (const itemId of activeWorkerItemIds) {
    const item = state.items.find((candidate) => candidate.id === itemId);
    if (item) {
      cancelAgentRun(itemId, item.profileId);
    }
  }

  const activePromises = [...activeWorkerPromises.values()];
  if (activePromises.length > 0) {
    let timeout: NodeJS.Timeout | null = null;
    await Promise.race([
      Promise.allSettled(activePromises),
      new Promise<void>((resolve) => {
        timeout = setTimeout(resolve, QUEUE_SHUTDOWN_TIMEOUT_MS);
        timeout.unref();
      }),
    ]);
    if (timeout) {
...
function readQueueItemUpdatedTime(item: CodexQueueItem): number {
...
function isQueueItemRelatedToWorkspace(
  item: CodexQueueItem,
  requestedQueueKey: string | null,
  resolvedSessionId: string | null
): boolean {
  const workspaceKeys = new Set(
    [requestedQueueKey, resolvedSessionId].filter((value): value is string => Boolean(value))
  );
  if (workspaceKeys.size === 0) {
    return false;
  }

  return workspaceKeys.has(item.queueKey)
    || Boolean(item.sessionId && workspaceKeys.has(item.sessionId));
}

/**
 * Keep every actionable queue record visible while bounding completed history.
 * Completed items are used only for recent completion transitions and draft
 * binding; the provider session timeline remains their durable history.
 */
export function selectCodexQueueWorkspaceItems(
  items: readonly CodexQueueItem[],
  requestedQueueKey: string | null,
  resolvedSessionId: string | null,
  options: {
    recentCompletedLimit?: number;
    relatedCompletedLimit?: number;
  } = {}
): CodexQueueItem[] {
  const recentCompletedLimit = Math.max(
    0,
    options.recentCompletedLimit ?? QUEUE_LIST_RECENT_COMPLETED_LIMIT
  );
  const relatedCompletedLimit = Math.max(
    0,
    options.relatedCompletedLimit ?? QUEUE_LIST_RELATED_COMPLETED_LIMIT
  );
  const completedNewestFirst = items
    .filter((item) => item.status === 'completed')
    .sort((left, right) => readQueueItemUpdatedTime(right) - readQueueItemUpdatedTime(left));
  ...
  completedNewestFirst
    .filter((item) => isQueueItemRelatedToWorkspace(item, requestedQueueKey, resolvedSessionId))
    .slice(0, relatedCompletedLimit)
    .forEach((item) => selectedIds.add(item.id));

  return items.filter((item) => selectedIds.has(item.id));
}

function toCodexQueueListItem(
  item: CodexQueueItem,
  detailsIncluded: boolean
): CodexQueueListItem {
  const compactForkContext = item.forkContext
    ? {
      sourceSessionId: item.forkContext.sourceSessionId,
      sourceTitle: item.forkContext.sourceTitle,
      sourceCwd: item.forkContext.sourceCwd,
  ...
  return {
    ...
    model: item.model,
    reasoningEffort: item.reasoningEffort,
    prompt: detailsIncluded ? item.prompt : '',
    promptPreview: item.promptPreview,
    forkContext: compactForkContext,
    attachments: item.attachments.map((attachment) => ({ ...attachment })),
    status: item.status,
    ...
    finalMessage: detailsIncluded ? item.finalMessage : null,
    error: item.error ? item.error.slice(0, QUEUE_LIST_ERROR_LIMIT) : null,
    attempts: item.attempts,
    scheduleMode: item.scheduleMode,
    recurringFrequency: item.recurringFrequency,
    recurringTimeZone: item.recurringTimeZone,
    lastRunAt: item.lastRunAt,
    lastRunStatus: item.lastRunStatus,
    stopPolicy: cloneCodexQueueStopPolicy(item.stopPolicy),
    stopDecisionForItemId: item.stopDecisionForItemId,
    continuationOfItemId: item.continuationOfItemId,
    detailsIncluded,
  };
}

export async function listCodexQueueWorkspaceItems(
  profileId: string | undefined,
  requestedQueueKeyInput: string | null | undefined
): Promise<CodexQueueWorkspaceList> {
  await refreshQueueState();

  const requestedQueueKey = normalizeSessionBindingKey(requestedQueueKeyInput);
  const resolvedSessionId = requestedQueueKey
    ? resolveSessionBindingFromState(requestedQueueKey, { fallbackToSelf: true })
    : null;
  const boundSessionId = requestedQueueKey
    && resolvedSessionId
    && resolvedSessionId !== requestedQueueKey
    ? resolvedSessionId
    : null;
  const profileItems = profileId
    ? state.items.filter((item) => item.profileId === profileId)
    : state.items;
  const selectedItems = selectCodexQueueWorkspaceItems(
    profileItems,
    requestedQueueKey,
    resolvedSessionId
  );
  ...
  return {
    items: sortQueueItems(selectedItems).map((item) => {
      const isRelated = isQueueItemRelatedToWorkspace(
        item,
        requestedQueueKey,
        resolvedSessionId
      );
      const detailsIncluded = isRelated
        && (item.status === 'failed' || item.status === 'cancelled');
      return toCodexQueueListItem(item, detailsIncluded);
    }),
    boundSessionId,
  };
}
