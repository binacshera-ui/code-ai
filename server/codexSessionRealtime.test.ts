...
test('session detail cache is reusable and live subscription invalidates it', async (t) => {
  ...
  const budgetSessionIds = Array.from(
    { length: 10 },
    (_, index) => `aaaaaaaa-bbbb-4ccc-8ddd-${String(index + 1).padStart(12, '0')}`
  );
  ...
  await Promise.all(budgetSessionIds.map(async (budgetSessionId, index) => {
    const budgetSessionPath = path.join(
      sessionDir,
      `rollout-2026-08-07T09-${String(index).padStart(2, '0')}-00-${budgetSessionId}.jsonl`
    );
    await fs.writeFile(budgetSessionPath, [
      JSON.stringify({
        type: 'session_meta',
        timestamp: `2026-08-07T09:${String(index).padStart(2, '0')}:00.000Z`,
        payload: {
          id: budgetSessionId,
          timestamp: `2026-08-07T09:${String(index).padStart(2, '0')}:00.000Z`,
          cwd: workspace,
      ...
      JSON.stringify({
        type: 'event_msg',
        timestamp: `2026-08-07T09:${String(index).padStart(2, '0')}:01.000Z`,
        payload: {
          type: 'user_message',
          message: `budget-${index}-${'x'.repeat(750_000)}`,
        },
      }),
      '',
    ].join('\n'), 'utf-8');
  ...
  process.env.CODEX_SESSION_DETAIL_CACHE_MAX_BYTES = String(32 * 1024 * 1024);
  ...
  for (const budgetSessionId of budgetSessionIds) {
    await service.getCodexSessionDetail(budgetSessionId, 'test-codex', { tail: 120 });
  }
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    if (await fs.stat(oldestBudgetCachePath).then(() => true).catch(() => false)) break;
    await new Promise((resolve) => setTimeout(resolve, 10));
  }
  const oldestBudgetPersisted = JSON.parse(await fs.readFile(oldestBudgetCachePath, 'utf-8'));
  oldestBudgetPersisted.detail.messages[0].text = 'loaded-from-disk-after-byte-budget-eviction';
  oldestBudgetPersisted.detail.timeline[0].text = 'loaded-from-disk-after-byte-budget-eviction';
  await fs.writeFile(oldestBudgetCachePath, JSON.stringify(oldestBudgetPersisted), 'utf-8');

  const evictedBudgetDetail = await service.getCodexSessionDetail(
    budgetSessionIds[0],
    'test-codex',
    { tail: 120 }
  );
  assert.equal(
    evictedBudgetDetail.timeline[0]?.text,
    'loaded-from-disk-after-byte-budget-eviction',
    'oldest detail should be evicted when the byte budget is exceeded'
  );
});
