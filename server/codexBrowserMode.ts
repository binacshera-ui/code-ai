...
function isBrowserSessionDirectory(targetPath: string): boolean {
  const relative = path.relative(BROWSER_MODE_SESSIONS_ROOT, path.resolve(targetPath));
  return Boolean(relative) && !relative.startsWith('..') && !path.isAbsolute(relative);
}

async function removeBrowserSessionDirectory(targetPath: string): Promise<void> {
  if (!isBrowserSessionDirectory(targetPath)) {
    return;
  }
...
export async function consumeSessionBrowserModeAfterDispatch(profileId: string, sessionKey: string): Promise<void> {
  ...
  delete state.browserModeByKey[key];
  await persistState();
  await removeBrowserSessionDirectory(current.sessionDir);
}
...
export async function deleteSessionBrowserMode(profileId: string, sessionKey: string): Promise<void> {
  ...
  delete state.browserModeByKey[key];
  await persistState();
  await removeBrowserSessionDirectory(current?.sessionDir || buildSessionDirs(profileId, sessionKey).sessionDir);
}
