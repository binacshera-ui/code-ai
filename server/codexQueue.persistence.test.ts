...
const triggerRuntime = await import('./codexSessionTriggerRuntime.js');
...
test('binds a started provider session without losing its original draft key', async () => {
  const item = await queue.enqueueCodexQueueItem({
    profileId: 'developer',
    queueKey: 'draft-visible-after-refresh',
    prompt: 'start a durable draft',
    scheduledAt: new Date(Date.now() + 60 * 60 * 1000).toISOString(),
  });

  const bound = await queue.bindCodexQueueItemToStartedSession(item.id, 'session-started-1');
  assert.equal(bound?.queueKey, 'draft-visible-after-refresh');
  assert.equal(bound?.sessionId, 'session-started-1');
  assert.equal(
    await queue.resolveCodexQueueSessionId('draft-visible-after-refresh'),
    'session-started-1'
  );
});

test('deduplicates retried session-trigger events without merging different incidents', async () => {
  const firstRequestId = triggerRuntime.buildSessionTriggerClientRequestId(
    'trigger-1',
    'failure-handler:incident-1'
  );
  const repeatedRequestId = triggerRuntime.buildSessionTriggerClientRequestId(
    'trigger-1',
    'failure-handler:incident-1'
  );
  const secondRequestId = triggerRuntime.buildSessionTriggerClientRequestId(
    'trigger-1',
    'failure-handler:incident-2'
  );

  assert.equal(firstRequestId, repeatedRequestId);
  assert.notEqual(firstRequestId, secondRequestId);

  const first = await queue.enqueueCodexQueueItem({
    profileId: 'developer',
    queueKey: 'session-trigger-dedupe',
    sessionId: 'session-trigger-dedupe',
    clientRequestId: firstRequestId,
    prompt: 'first delivery',
    scheduledAt: new Date(Date.now() + 60 * 60 * 1000).toISOString(),
  });
  const repeated = await queue.enqueueCodexQueueItem({
    profileId: 'developer',
    queueKey: 'session-trigger-dedupe',
    sessionId: 'session-trigger-dedupe',
    clientRequestId: repeatedRequestId,
    prompt: 'retry after an ambiguous timeout',
    scheduledAt: new Date(Date.now() + 60 * 60 * 1000).toISOString(),
  });
  const second = await queue.enqueueCodexQueueItem({
    profileId: 'developer',
    queueKey: 'session-trigger-dedupe',
    sessionId: 'session-trigger-dedupe',
    clientRequestId: secondRequestId,
    prompt: 'different incident',
    scheduledAt: new Date(Date.now() + 60 * 60 * 1000).toISOString(),
  });
