...
export const SHARED_CONVERSATION_PROJECTION_VERSION = 'transcript-markdown-v1';
...
export function sanitizeSharedConversationText(value: string | null | undefined): string {
  return String(value || '')
    .replaceAll('\r\n', '\n')
    .replaceAll('\r', '\n')
...
function selectedConversationEntries(session: CodexSessionDetail): CodexTimelineEntry[] {
  return session.timeline.filter((entry) => isUserPrompt(entry) || isFinalAssistantResponse(entry));
}

export function buildSharedConversationSourceRevision(session: CodexSessionDetail): string {
  const relevantEntries = selectedConversationEntries(session);
  return createHash('sha256').update(JSON.stringify({
    projectionVersion: SHARED_CONVERSATION_PROJECTION_VERSION,
    sessionId: session.id,
    title: normalizeSingleLine(session.title, 'שיחה ללא כותרת'),
    messages: relevantEntries.map((entry) => ({
      id: entry.id,
      role: entry.role,
      kind: entry.kind,
      timestamp: formatMessageTimestamp(entry.timestamp) || entry.timestamp,
      text: sanitizeSharedConversationText(entry.text),
    })),
  }), 'utf8').digest('hex');
}
...
export function buildSharedConversationMarkdown(
  ...
  const sourceRevision = buildSharedConversationSourceRevision(session);
  const lines = [
    `# ${title}`,
    '',
    ...
    `- גרסת הקרנה: \`${SHARED_CONVERSATION_PROJECTION_VERSION}\``,
    `- גרסת מקור: \`${sourceRevision}\``,
  ...
  for (const entry of relevantEntries) {
    ...
    if (isUserPrompt(entry)) {
      ...
      lines.push(
        ...
        '',
        sanitizeSharedConversationText(entry.text),
        '',
      );
      continue;
    }
