...
const MAX_UNTRACKED_SNAPSHOT_FILE_BYTES = 16 * 1024 * 1024;
const MAX_UNTRACKED_SNAPSHOT_TOTAL_BYTES = 256 * 1024 * 1024;
const STALE_CAPTURE_MAX_AGE_MS = 24 * 60 * 60 * 1000;
const STALE_CAPTURE_PRUNE_INTERVAL_MS = 60 * 60 * 1000;
const FILE_PATH_REGEX = /(?:^|[\s(])((?:\/|\.\/|\.\.\/)[^\s'")]+|[A-Za-z0-9._-]+\.[A-Za-z0-9._/-]+)/g;
const ACTIVE_CAPTURE_TEMP_ROOTS = new Set<string>();

let lastStaleCapturePruneAt = 0;
...
function buildTrackedDiffText(
  ...
  const clipped = clipDiffText(diffOutput);
  return {
...
async function snapshotUntrackedFiles(
  ...
  for (const file of files) {
    ...
    const canSnapshot = stat.size <= MAX_UNTRACKED_SNAPSHOT_FILE_BYTES
      && capturedBytes + stat.size <= MAX_UNTRACKED_SNAPSHOT_TOTAL_BYTES;
    const destinationPath = canSnapshot ? path.join(snapshotDir, file.relativePath) : null;
    if (destinationPath) {
...
async function pruneStaleCaptureDirectories(): Promise<void> {
  const now = Date.now();
  if (now - lastStaleCapturePruneAt < STALE_CAPTURE_PRUNE_INTERVAL_MS) {
    return;
  }
  lastStaleCapturePruneAt = now;

  await fs.mkdir(SESSION_CHANGE_TEMP_ROOT, { recursive: true });
  const entries = await fs.readdir(SESSION_CHANGE_TEMP_ROOT, { withFileTypes: true });
  await Promise.all(entries
    .filter((entry) => entry.isDirectory())
    .map(async (entry) => {
      const capturePath = path.join(SESSION_CHANGE_TEMP_ROOT, entry.name);
      if (ACTIVE_CAPTURE_TEMP_ROOTS.has(capturePath)) {
        return;
      }
      const stat = await fs.stat(capturePath).catch(() => null);
      if (stat && now - stat.mtimeMs >= STALE_CAPTURE_MAX_AGE_MS) {
        await fs.rm(capturePath, { recursive: true, force: true }).catch(() => undefined);
      }
    }));
}

function buildOmittedSnapshotRecord(
  capture: SessionChangeCapture,
  relativePath: string,
  status: SessionChangeFileStatus,
): SessionChangeFileRecord {
  return {
    id: `${relativePath}:${status}-snapshot-omitted`,
    path: relativePath,
    ...
    deletions: 0,
    isBinary: true,
    diffText: 'Large or overflow file changed; content snapshot was intentionally omitted.',
    diffTruncated: true,
  };
}
...
export async function beginSessionChangeCapture(input: {
  ...
  await pruneStaleCaptureDirectories().catch(() => undefined);
  ...
  ACTIVE_CAPTURE_TEMP_ROOTS.add(tempRoot);
  try {
    ...
    ACTIVE_CAPTURE_TEMP_ROOTS.delete(tempRoot);
...
export async function finalizeSessionChangeCapture(
  ...
  if (!input.entryId) {
    if (options.cleanup !== false) {
      ACTIVE_CAPTURE_TEMP_ROOTS.delete(capture.tempRoot);
    ...
    if (capture.repoRoot) {
      ...
      for (const relativePath of allUntrackedPaths) {
        ...
        if (startFile && endFile) {
          if (!startFile.snapshotPath) {
            const endStat = await fs.stat(endFile.absolutePath).catch(() => null);
            if (endStat && endStat.size === startFile.size && endStat.mtimeMs === startFile.mtimeMs) {
              continue;
            }
            files.push(buildOmittedSnapshotRecord(capture, relativePath, 'modified'));
            continue;
          }
        ...
        if (!startFile && endFile) {
          const endStat = await fs.stat(endFile.absolutePath).catch(() => null);
          if (endStat && endStat.size > MAX_UNTRACKED_SNAPSHOT_FILE_BYTES) {
            files.push(buildOmittedSnapshotRecord(capture, relativePath, 'created'));
            continue;
          }
        ...
        if (startFile && !endFile) {
          if (!startFile.snapshotPath) {
            files.push(buildOmittedSnapshotRecord(capture, relativePath, 'deleted'));
            continue;
          }
...
async function restorePathToBaseline(capture: SessionChangeCapture, relativePath: string): Promise<void> {
  ...
  if (startUntracked) {
    if (!startUntracked.snapshotPath) {
      throw new Error(`Cannot restore oversized untracked file without a content snapshot: ${relativePath}`);
    }
    await fs.mkdir(path.dirname(absolutePath), { recursive: true });
