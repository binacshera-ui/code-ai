...
// A count-only limit is unsafe for this application: one user's rollout files
// can be hundreds of megabytes while another user's are tiny. Budget the
// parsed detail objects as well so a handful of very large conversations
// cannot fill V8's heap even though the entry-count limit has not been hit.
const SESSION_DETAIL_CACHE_MAX_BYTES = Math.max(
  32 * 1024 * 1024,
  Number(process.env.CODEX_SESSION_DETAIL_CACHE_MAX_BYTES) || 256 * 1024 * 1024
);
...
let sessionDetailCacheEstimatedBytes = 0;
...
function extractCodexEventErrorMessage(payload: any): string | null {
  const candidates = [
    typeof payload?.error === 'string' ? payload.error : null,
    typeof payload?.error?.message === 'string' ? payload.error.message : null,
    typeof payload?.message === 'string' && /(?:error|fail|invalid|denied|timeout|not found)/i.test(payload.message)
      ? payload.message
      : null,
  ];
...
function extractCodexStructuredError(row: any): string | null {
  ...
  const hints = [
    ...
    typeof row.payload?.error === 'string' ? row.payload.error : null,
    typeof row.payload?.error?.message === 'string' ? row.payload.error.message : null,
    typeof row.payload?.message === 'string' && /(error|failed|invalid|denied|timeout|not found)/i.test(row.payload.message)
      ? row.payload.message
      : null,
...
export function sanitizeCodexCliFailure(
  ...
  if (/thread-store conflict|already has an active writer/i.test(failureText)) {
    const threadId = failureText.match(/\bthread\s+([0-9a-f-]{8,})\b/i)?.[1] || null;
    const conversationLabel = threadId
      ? `Codex conversation "${threadId}"`
      : 'This Codex conversation';
    return [
      `${conversationLabel} is already open in another Codex process for profile "${profile.label}".`,
      `Only this conversation is locked; the "${profile.label}" profile and its other conversations remain available.`,
      'Wait for the existing run to finish or stop that exact run before retrying this conversation.',
    ].join(' ');
  }

  if (
    /Codex cannot access session files|Permission denied|Operation not permitted|os error 13|\bEACCES\b|\bEPERM\b/i
      .test(failureText)
  ) {
    return [
      `Codex profile "${profile.label}" cannot read or write ${profile.codexHome}.`,
      'Check the configured profile runtime identity and filesystem permissions; code-ai will not propose a broad recursive ownership change automatically.',
    ].join(' ');
  }
...
async function extractSessionSummaryHints(
  ...
  for (const line of [...tailLines].reverse()) {
    ...
    if (eventType === 'task_complete') {
      ...
      if (taskError) {
        endPreview = trimPreview(taskError);
        break;
      }
    }
        ...
        if (eventType === 'task_complete') {
          ...
          if (taskError) {
            pushTimeline({
              ...
              timestamp,
              title: 'המשימה נכשלה',
              subtitle: clipLongText(taskError, 1_200),
              status: 'failed',
            });
            preview = taskError;
            continue;
...
function estimateSessionDetailMemoryBytes(serializedBytes: number): number {
  // Parsed JSON retains strings plus object/array bookkeeping. Three times the
  // UTF-8 payload is deliberately conservative for mixed Hebrew/English text.
  return Math.max(
    1,
    Math.ceil(serializedBytes * SESSION_DETAIL_CACHE_OBJECT_OVERHEAD_MULTIPLIER)
  );
}

function deleteSessionDetailMemoryCacheKey(key: string): void {
  const current = sessionDetailCache.get(key);
  if (!current) return;
  sessionDetailCache.delete(key);
  sessionDetailCacheEstimatedBytes = Math.max(
    0,
    sessionDetailCacheEstimatedBytes - current.estimatedBytes
  );
}

function setSessionDetailMemoryCache(
  key: string,
  record: SessionDetailCacheRecord
): void {
  deleteSessionDetailMemoryCacheKey(key);
  sessionDetailCache.set(key, record);
  sessionDetailCacheEstimatedBytes += record.estimatedBytes;
  pruneSessionDetailMemoryCache();
}

function pruneSessionDetailMemoryCache(): void {
  if (
    sessionDetailCache.size <= SESSION_DETAIL_CACHE_LIMIT
    && sessionDetailCacheEstimatedBytes <= SESSION_DETAIL_CACHE_MAX_BYTES
  ) {
    return;
  }
...
async function readPersistedSessionDetail(
  ...
  variant: string,
  sourceSignature: string
): Promise<{ detail: CodexSessionDetail; estimatedBytes: number } | null> {
  const cachePath = buildSessionDetailDiskPath(profileId, sessionId, variant);
  try {
    const raw = await fs.readFile(cachePath, 'utf-8');
    const parsed = JSON.parse(raw) as PersistedSessionDetailRecord;
    if (
    ...
    return {
      ...
      estimatedBytes: estimateSessionDetailMemoryBytes(Buffer.byteLength(raw, 'utf-8')),
    };
  } catch (error: any) {
...
async function persistSessionDetail(
  ...
  await fs.writeFile(temporaryPath, serialized, { encoding: 'utf-8', mode: 0o600 });
...
async function getCachedSessionDetail(
  ...
  setSessionDetailMemoryCache(key, {
    sourceSignature,
    sourceIdentity,
    sourceSize,
...
async function putCachedSessionDetail(
  ...
  const serialized = JSON.stringify({
    ...
    sourceSize,
    detail,
  } satisfies PersistedSessionDetailRecord);
  setSessionDetailMemoryCache(key, {
    sourceSignature,
    sourceIdentity,
    sourceSize,
    detail,
    estimatedBytes: estimateSessionDetailMemoryBytes(Buffer.byteLength(serialized, 'utf-8')),
    lastAccessedAt: Date.now(),
  });
...
export function cancelCodexRun(runId: string): boolean {
  ...
  return terminateProviderProcessTree(activeRun.child);
}
...
export async function runCodexPrompt(
  ...
  const effectivePromptText = options.contextPrefix?.trim()
    ...
    : promptText;
  const queueKey = buildProviderRunQueueKey(profile.id, sessionId, options.runId);
  ...
  return queueBySessionKey(queueKey, async () => {
    ...
    const args = buildCodexInvocationArgs(permissionArgs, sessionId, imagePaths, executionConfig);
      ...
      child.stdout.on('data', (chunk: string) => {
        ...
        while (stdoutBuffer.includes('\n')) {
          ...
          if (row.type === 'thread.started' && typeof row.thread_id === 'string') {
            createdSessionId = row.thread_id.trim();
            if (createdSessionId && createdSessionId !== announcedSessionId) {
              announcedSessionId = createdSessionId;
              notifyProviderSessionStarted(options.onSessionStarted, createdSessionId, 'Codex');
            }
          }
