...
const contract = JSON.parse(await readFile(resolve(runtimeDir, 'ux_tools.json'), 'utf8'));
...
async function loadBridge() {
  ...
  if (!fileStat.isFile() || (fileStat.mode & 0o077) !== 0) {
    throw new Error('UX bridge discovery file must be owner-only');
  }
  ...
  if (!['127.0.0.1', 'localhost', '::1'].includes(bridgeUrl.hostname)
    ...
    throw new Error('UX bridge discovery data is invalid');
  }
  ...
  try {
    if (message.method === 'initialize') {
      sendResult(id, {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'code-ai-gemini-ux-partner', version: '1.0.0' },
        instructions: 'Gemini provides product and UX judgment only. Codex owns all product decisions, implementation, code changes and validation.',
      });
    ...
    if (id !== undefined) {
      sendResult(id, {
        content: [{ type: 'text', text: JSON.stringify({ error_code: 'UX_MCP_FAILURE', message: String(error?.message || error), is_retryable: true }, null, 2) }],
        structuredContent: { error_code: 'UX_MCP_FAILURE', message: String(error?.message || error), is_retryable: true },
        isError: true,
      });
    }
  }
