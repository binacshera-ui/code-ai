...
export function buildSessionPersonalChromePromptAdditions(mode: CodexSessionPersonalChromeMode) {
  ...
  return [
    ...
    'browser_tabs מציג את כל הטאבים הגלויים להתקנת התוסף בפרופיל Chrome הנוכחי. מותר לקרוא snapshot, צילום, DOM, קונסול ורשת גם מטאב שאינו משויך, אבל אסור לשנות אותו.',
    'כל פעולה שמשנה דף או טאב מותרת רק בתוך קבוצת העבודה של הסשן. לפני פעולה בטאב קיים שאינו משויך, השתמש ב-browser_workspace עם action=attach; לחלופין צור או שכפל טאב דרך browser_tab_control, והוא ישויך ויקובץ אוטומטית.',
    'פתיחה, שכפול, ניווט, גלילה, צילום וקריאת דפים מתבצעים ברקע ואסור להם להעביר את הפוקוס מהטאב שבו המשתמש נמצא. השתמש ב-browser_tab_control action=activate או browser_workspace action=focus רק אם המשתמש ביקש במפורש להציג את הטאב בחזית.',
    'אם מתקבלת TAB_NOT_ASSIGNED, אל תנסה לעקוף אותה ואל תעבור לטאב אחר במקרה: הצג למשתמש איזה טאב דורש שיוך, או שייך/שכפל אותו בהתאם לבקשה.',
    'פרופילי Chrome מבודדים זה מזה. התקנת תוסף בפרופיל אחד אינה רואה פרופיל אחר; כל פרופיל נוסף חייב התקנה ואישור נפרדים ויופיע ב-CODE-AI כמכשיר Chrome נפרד.',
