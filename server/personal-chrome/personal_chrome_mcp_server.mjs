...
const tools = [
  ...
  ['browser_tabs', 'List all visible tabs and their current-session workspace membership.', { includePinned: { type: 'boolean', default: true }, scope: { type: 'string', enum: ['all', 'workspace'], default: 'all' } }, []],
  ['browser_workspace', 'Inspect or manage the current session Chrome workspace. Duplicate stays in the background; focus is explicitly foreground.', { action: { type: 'string', enum: ['status', 'attach', 'detach', 'duplicate', 'focus', 'rename'] }, tabId, title: { type: 'string', minLength: 1, maxLength: 80 } }, ['action']],
  ['browser_tab_control', 'Create, duplicate, reload, or close a managed workspace tab in the background. Activate is the only explicitly foreground action.', { action: { type: 'string', enum: ['activate', 'new', 'duplicate', 'reload', 'close'] }, tabId, url: { type: 'string' } }, ['action']],
  ['browser_navigate', 'Navigate a tab in the background without changing the user\'s active tab, and optionally wait for readiness.', { tabId, url: { type: 'string' }, waitUntil: { type: 'string', enum: ['none', 'domcontentloaded', 'complete'], default: 'domcontentloaded' }, timeoutMs: { type: 'integer', minimum: 1000, maximum: 60000 } }, ['url']],
