...
function getDefaultProfiles(appRoot: string, configuredWorkspaceRoot?: string): CodexProfileConfig[] {
  const workspaceRoot = configuredWorkspaceRoot || inferWorkspaceRoot(appRoot);
...
function loadProfiles(appRoot: string, workspaceRoot?: string): CodexProfileConfig[] {
  ...
  if (!raw) {
    return finalizeProfiles(getDefaultProfiles(appRoot, workspaceRoot));
  }
...
const PROFILES = loadProfiles(APP_ROOT, WORKSPACE_ROOT);
