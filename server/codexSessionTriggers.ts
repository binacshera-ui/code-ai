...
async function ensureStorageRoot() {
  await fs.mkdir(CODEX_APP_CONFIG.storageRoot, { recursive: true, mode: 0o700 });
}
...
async function persistState() {
  ...
  persistTail = persistTail.then(async () => {
    ...
    const temporary = `${TRIGGERS_FILE}.${process.pid}.${randomUUID()}.tmp`;
    try {
      await fs.writeFile(temporary, snapshot, { encoding: 'utf-8', mode: 0o600 });
      await fs.rename(temporary, TRIGGERS_FILE);
      await fs.chmod(TRIGGERS_FILE, 0o600);
    } finally {
