...
export const PERSONAL_CHROME_TOOLS: readonly PersonalChromeToolDefinition[] = [
  ...
  },
  {
    ...
    description: 'List every tab visible to this Chrome-profile installation, including whether it belongs to the current session workspace. Unmanaged tabs remain readable but cannot be mutated.',
    scope: 'read', mutating: false, risky: false,
    inputSchema: {
      type: 'object', additionalProperties: false,
      properties: {
        includePinned: { type: 'boolean', default: true },
        scope: { type: 'string', enum: ['all', 'workspace'], default: 'all' },
      },
    },
  },
  {
    name: 'browser_workspace',
    title: 'Manage the session Chrome workspace',
    description: 'Inspect or manage the current session tab workspace. Attach an existing tab, detach it, duplicate a managed tab in the background, explicitly focus the group, or rename the Chrome tab group.',
    scope: 'write', mutating: true, risky: false,
    inputSchema: {
      type: 'object', required: ['action'], additionalProperties: false,
      properties: {
        action: { type: 'string', enum: ['status', 'attach', 'detach', 'duplicate', 'focus', 'rename'] },
        tabId: TAB_ID,
  ...
  },
  {
    ...
    description: 'Activate, create, duplicate, reload, or close a tab in the current session workspace. New and duplicated tabs open in the background and are grouped automatically; activate is the only action that may move user focus.',
    scope: 'write', mutating: true, risky: false,
    inputSchema: {
      type: 'object', required: ['action'], additionalProperties: false,
      properties: {
        action: { type: 'string', enum: ['activate', 'new', 'duplicate', 'reload', 'close'] },
        tabId: TAB_ID,
  ...
  },
  {
    ...
    description: 'Navigate the bound tab to an http/https URL in the background, without activating it or changing the user\'s current tab, and wait for the requested readiness state.',
    scope: 'write', mutating: true, risky: false,
    inputSchema: {
