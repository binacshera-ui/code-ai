...
function buildGeminiProcessEnv(
  ...
  return buildIsolatedGeminiProcessEnv(
    process.env,
    envValues,
    {
      HOME: homePath,
...
export async function runGeminiPrompt(
  ...
  const sessionKey = buildProviderRunQueueKey(profile.id, sessionId, options.runId);
          ...
          while (stdoutBuffer.includes('\n')) {
            ...
            if (row.type === 'init') {
              ...
              if (createdSessionId && createdSessionId !== announcedSessionId) {
                ...
                notifyProviderSessionStarted(options.onSessionStarted, createdSessionId, 'Gemini');
              }
...
export function cancelGeminiRun(runId: string): boolean {
  ...
  return terminateProviderProcessTree(activeRun.child, 1_200);
}
