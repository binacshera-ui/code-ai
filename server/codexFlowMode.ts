...
export async function buildSessionFlowModePromptAdditions(profileId: string, sessionKey: string): Promise<string | null> {
  ...
  return [
    ...
    'הסכימה: {schemaVersion:1,id,title,subtitle,summary,direction:"rtl"|"ltr",groups:[{id,title,description,color:"sky"|"mint"|"violet"|"rose"|"amber"|"slate"}],nodes:[{id,title,summary,description,kind:"system"|"ui"|"api"|"service"|"worker"|"database"|"queue"|"container"|"external"|"file"|"decision"|"person"|"other",ownership:"repository"|"external"|"managed"|"infrastructure"|"unknown",status:"active"|"planned"|"risk"|"unknown",groupId:null|string,technology,runtime,repositoryPath,externalUrl,tags:string[],evidence:[{id,kind:"path"|"url"|"runtime"|"note",label,value}],position:null|{x,y}}],edges:[{id,source,target,label,description,kind:"data"|"request"|"event"|"dependency"|"control"|"deploy"|"other"}],updatedAt}.',
