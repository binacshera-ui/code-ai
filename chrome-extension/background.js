...
const WORKSPACES_STORAGE_KEY = 'codeAiPersonalChromeWorkspacesV1';
const PANEL_ANCHOR_STORAGE_KEY = 'codeAiPersonalChromePanelAnchorV1';
...
function normalizeSessionContext(value) {
  ...
  return {
    ...
    sessionKey,
    bindingId,
    sessionTitle: typeof value.sessionTitle === 'string' ? value.sessionTitle.trim().slice(0, 80) : '',
  };
}
...
function workspaceKey(session) {
  ...
  return `${normalized.serverId}:${normalized.profileId}:${normalized.sessionKey}`;
}

function shortSessionLabel(sessionKey) {
  const clean = String(sessionKey || '').replace(/^draft:/, '').replace(/[^\p{L}\p{N}._-]+/gu, ' ').trim();
  return clean.length > 24 ? `${clean.slice(0, 21)}…` : clean || 'שיחה';
}

function stableWorkspaceColor(key) {
  const colors = ['blue', 'cyan', 'green', 'yellow', 'orange', 'pink', 'purple', 'red'];
  let hash = 0;
  for (const character of String(key || '')) hash = ((hash << 5) - hash + character.codePointAt(0)) | 0;
...
function createWorkspace(session) {
  ...
  if (!normalized || !key) throw errorWithCode('The browser command is not bound to a CODE-AI session.', 'SESSION_NOT_BOUND');
  return {
    key,
    ...
    title: `CODE-AI · ${shortSessionLabel(normalized.sessionTitle || normalized.sessionKey)}`,
    color: stableWorkspaceColor(key),
    tabIds: [],
    groupIds: {},
...
function workspaceForSession(session, create = false) {
  const key = workspaceKey(session);
  if (!key) return null;
  let workspace = workspaces.get(key) || null;
  if (!workspace && create) {
    workspace = createWorkspace(session);
    workspaces.set(key, workspace);
  }
  const normalized = normalizeSessionContext(session);
  if (workspace && normalized?.bindingId && workspace.bindingId !== normalized.bindingId) workspace.bindingId = normalized.bindingId;
  if (workspace && normalized?.sessionTitle) workspace.title = `CODE-AI · ${shortSessionLabel(normalized.sessionTitle)}`;
  return workspace;
}
...
async function persistWorkspaces() {
  await chrome.storage.local.set({ [WORKSPACES_STORAGE_KEY]: [...workspaces.values()] });
}

async function restoreWorkspaces() {
  const stored = await chrome.storage.local.get(WORKSPACES_STORAGE_KEY).catch(() => ({}));
  const values = Array.isArray(stored?.[WORKSPACES_STORAGE_KEY]) ? stored[WORKSPACES_STORAGE_KEY] : [];
  for (const value of values) {
    if (!value?.key || !Array.isArray(value.tabIds)) continue;
    workspaces.set(value.key, {
      ...value,
      tabIds: value.tabIds.filter(Number.isInteger),
      groupIds: value.groupIds && typeof value.groupIds === 'object' ? value.groupIds : {},
      color: value.color || stableWorkspaceColor(value.key),
    });
  }
...
async function panelAnchorTab() {
  if (panelAnchor?.tabId) {
    const tab = await chrome.tabs.get(panelAnchor.tabId).catch(() => null);
    if (tab) return tab;
  }
  return activeTab();
}

function publicTab(tab, session = activeSessionContext) {
  const owner = workspaceForTab(tab.id);
  const current = workspaceForSession(session);
  return {
    id: tab.id,
    ...
    currentWorkspace: Boolean(owner && current && owner.key === current.key),
    actionAllowed: Boolean(owner && current && owner.key === current.key),
    workspace: owner ? { key: owner.key, title: owner.title, color: owner.color } : null,
  };
}

async function syncWorkspaceGroups(workspace) {
  const tabs = (await Promise.all(workspace.tabIds.map((tabId) => chrome.tabs.get(tabId).catch(() => null)))).filter(Boolean);
  workspace.tabIds = tabs.map((tab) => tab.id);
  if (workspace.lastActiveTabId && !workspace.tabIds.includes(workspace.lastActiveTabId)) workspace.lastActiveTabId = workspace.tabIds[0] || null;
  const byWindow = new Map();
  for (const tab of tabs) {
    const entries = byWindow.get(tab.windowId) || [];
    entries.push(tab);
    byWindow.set(tab.windowId, entries);
  }
  const nextGroupIds = {};
  for (const [windowId, windowTabs] of byWindow) {
    const tabIds = windowTabs.map((tab) => tab.id);
    let groupId = Number(workspace.groupIds?.[windowId]);
    const existing = Number.isInteger(groupId) && groupId >= 0
      ? await chrome.tabGroups.get(groupId).catch(() => null)
      : null;
    groupId = await chrome.tabs.group(existing?.windowId === windowId ? { groupId, tabIds } : { tabIds });
    await chrome.tabGroups.update(groupId, { title: workspace.title, color: workspace.color, collapsed: false });
    nextGroupIds[windowId] = groupId;
  }
  workspace.groupIds = nextGroupIds;
...
async function reconcileWorkspaces() {
  const existingTabs = new Set((await chrome.tabs.query({})).map((tab) => tab.id));
  for (const workspace of workspaces.values()) {
    workspace.tabIds = workspace.tabIds.filter((tabId) => existingTabs.has(tabId));
    await syncWorkspaceGroups(workspace).catch(() => undefined);
  }
  await persistWorkspaces();
}

async function attachTabToWorkspace(tabId, session = activeSessionContext) {
  const workspace = workspaceForSession(session, true);
  const tab = await chrome.tabs.get(tabId).catch(() => null);
  if (!tab) throw errorWithCode(`Chrome tab ${tabId} no longer exists.`, 'TAB_NOT_BOUND');
  const previousOwner = workspaceForTab(tabId);
  if (previousOwner && previousOwner.key !== workspace.key) {
    previousOwner.tabIds = previousOwner.tabIds.filter((candidate) => candidate !== tabId);
    await syncWorkspaceGroups(previousOwner);
  }
  if (tab.pinned) await chrome.tabs.update(tabId, { pinned: false });
  if (!workspace.tabIds.includes(tabId)) workspace.tabIds.push(tabId);
  workspace.lastActiveTabId = tabId;
  await syncWorkspaceGroups(workspace);
  return { workspace: publicWorkspace(workspace), tab: publicTab(await chrome.tabs.get(tabId), session) };
}

async function createBackgroundWorkspaceTab({ url = 'about:blank', sourceTab = null, session = activeSessionContext } = {}) {
  const workspace = workspaceForSession(session, true);
  const lastWorkspaceTab = !sourceTab && workspace.lastActiveTabId
    ? await chrome.tabs.get(workspace.lastActiveTabId).catch(() => null)
    : null;
  const referenceTab = sourceTab || lastWorkspaceTab;
  const createOptions = {
    url,
    active: false,
    ...(referenceTab?.windowId ? { windowId: referenceTab.windowId } : {}),
    ...(Number.isInteger(referenceTab?.index) ? { index: referenceTab.index + 1 } : {}),
  };
  const tab = await chrome.tabs.create(createOptions);
  await attachTabToWorkspace(tab.id, session);
  return tab;
}

async function duplicateWorkspaceTabInBackground(tab, session = activeSessionContext) {
  const url = tab?.url || tab?.pendingUrl || 'about:blank';
  return createBackgroundWorkspaceTab({ url, sourceTab: tab, session });
}

async function detachTabFromWorkspace(tabId, session = activeSessionContext) {
  const workspace = workspaceForSession(session);
  if (!workspace || !workspace.tabIds.includes(tabId)) throw errorWithCode('This tab does not belong to the current session workspace.', 'TAB_NOT_BOUND');
  workspace.tabIds = workspace.tabIds.filter((candidate) => candidate !== tabId);
  await chrome.tabs.ungroup(tabId).catch(() => undefined);
  await syncWorkspaceGroups(workspace);
  return { workspace: publicWorkspace(workspace), tabId, detached: true };
}

function publicWorkspace(workspace = workspaceForSession(activeSessionContext)) {
  ...
  return {
    ...
    groupIds: Object.values(workspace.groupIds || {}).filter(Number.isInteger),
    lastActiveTabId: workspace.lastActiveTabId,
  };
}

async function workspaceState(session = activeSessionContext, { tabScope = 'all' } = {}) {
  const tabs = await chrome.tabs.query({});
  const workspace = workspaceForSession(session);
  const listedTabs = tabs.map((tab) => publicTab(tab, session));
  const workspaceTabs = listedTabs.filter((tab) => tab.currentWorkspace);
  return {
    workspace: publicWorkspace(workspace),
    anchorTabId: panelAnchor?.tabId || null,
    tabs: tabScope === 'workspace' ? workspaceTabs : listedTabs,
    tabCounts: {
      visible: listedTabs.length,
      workspace: workspaceTabs.length,
      actionable: workspaceTabs.filter((tab) => tab.actionAllowed).length,
    },
    profileIsolation: {
      installationId,
      note: 'Chrome isolates tabs by browser profile. Install and enroll this extension separately in every Chrome profile you want CODE-AI to control.',
    },
  };
}

function broadcastWorkspaceState(session = activeSessionContext) {
  void workspaceState(session).then((state) => broadcast({ type: 'WORKSPACE_CHANGED', state })).catch(() => undefined);
}
  ...
  activeSocket.addEventListener('message', (event) => {
    ...
    if (message.type === 'auth_ok') {
      ...
      sendSocket({ type: 'event', version: PROTOCOL_VERSION, name: 'capabilities', payload: [
        'tabs', 'tab-groups', 'session-workspaces', 'read-all-tabs', 'mutate-managed-tabs-only', 'scripting', 'debugger', 'dom-selection-v2', 'region-selection-v2', 'selection-context', 'screenshot', 'console', 'network', 'tab-scoped-side-panel',
      ] });
      return;
    }
...
async function resolveTab(args = {}, session = activeSessionContext, options = {}) {
  ...
  if (Number.isInteger(explicit) && explicit >= 0) {
    ...
    if (options.requireManaged) assertManagedTab(tab.id, session);
    return tab;
  }
  if (options.requireManaged) {
    const workspace = workspaceForSession(session);
    if (!workspace?.tabIds.length) throw errorWithCode('No tab is assigned to the current CODE-AI session workspace.', 'TAB_NOT_BOUND');
    const active = (await chrome.tabs.query({ active: true, currentWindow: true }))[0];
    const candidateId = active?.id && workspace.tabIds.includes(active.id)
      ? active.id
      : workspace.lastActiveTabId && workspace.tabIds.includes(workspace.lastActiveTabId)
        ? workspace.lastActiveTabId
        : workspace.tabIds[0];
    const tab = await chrome.tabs.get(candidateId).catch(() => null);
    if (!tab) throw errorWithCode('The assigned Chrome tab is no longer open.', 'TAB_NOT_BOUND');
    return tab;
  }
  return activeTab();
}

function assertManagedTab(tabId, session = activeSessionContext) {
  const workspace = workspaceForSession(session);
  if (!workspace?.tabIds.includes(tabId)) {
    throw errorWithCode('This operation is blocked because the tab is visible but not assigned to the current CODE-AI session. Attach or duplicate it first.', 'TAB_NOT_ASSIGNED', { tabId });
  }
  workspace.lastActiveTabId = tabId;
  return workspace;
}

async function resolveTargetTab(args = {}, session, options = {}) {
  ...
  if (!selectionId) return resolveTab(args, session, options);
...
chrome.tabs.onRemoved.addListener((tabId) => {
  ...
  const workspace = workspaceForTab(tabId);
  if (workspace) {
    workspace.tabIds = workspace.tabIds.filter((candidate) => candidate !== tabId);
    void syncWorkspaceGroups(workspace);
  }
  if (panelAnchor?.tabId === tabId) {
    panelAnchor = null;
    void chrome.storage.session.remove(PANEL_ANCHOR_STORAGE_KEY);
  }
});
...
chrome.tabs.onCreated.addListener(() => scheduleWorkspaceBroadcast());
chrome.tabs.onUpdated.addListener(() => scheduleWorkspaceBroadcast());
chrome.tabs.onMoved.addListener(() => scheduleWorkspaceBroadcast());
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message?.type === 'PANEL_GET_STATE') {
    void settingsReady
      .then(async () => sendResponse({ ...statusSnapshot(), settings, pendingApproval: currentApproval(), selections: selectionsForSession(activeSessionContext), workspaceState: await workspaceState() }))
  ...
  if (message?.type === 'PANEL_PICK') {
    void panelAnchorTab().then(async (tab) => {
      await attachTabToWorkspace(tab.id);
      return startPicker(tab, message.mode, message.mode === 'region_picker' ? 'בחר אזור חכם עבור CODE-AI' : 'בחר מוקד חכם עבור CODE-AI');
    }).then((selection) => sendResponse({ ok: true, selection, selections: selectionsForSession(activeSessionContext) })).catch((error) => sendResponse({ ok: false, error: error.message || String(error), code: error.code || 'SELECTION_FAILED' }));
    return true;
  }
  if (message?.type === 'PANEL_WORKSPACE_STATE') {
    void settingsReady.then(() => workspaceState()).then((state) => sendResponse({ ok: true, ...state })).catch((error) => sendResponse({ ok: false, error: error.message || String(error), code: error.code || 'WORKSPACE_FAILED' }));
    return true;
  }
  if (message?.type === 'PANEL_WORKSPACE_ACTION') {
    void settingsReady.then(async () => {
      const action = String(message.action || '');
      const tabId = Number(message.tabId);
      if (action === 'attach') return attachTabToWorkspace(tabId);
      if (action === 'detach') return detachTabFromWorkspace(tabId);
      if (action === 'activate') {
        assertManagedTab(tabId);
      ...
      if (action === 'duplicate') {
        ...
        const duplicated = await duplicateWorkspaceTabInBackground(source);
        return { duplicated: true, tab: publicTab(duplicated) };
      }
      if (action === 'close') {
        assertManagedTab(tabId);
      ...
      throw errorWithCode('Unsupported workspace action.', 'INVALID_ARGUMENT');
    }).then(async (result) => sendResponse({ ok: true, result, ...(await workspaceState()) })).catch((error) => sendResponse({ ok: false, error: error.message || String(error), code: error.code || 'WORKSPACE_FAILED' }));
    return true;
  }
...
async function syncActiveSession(context) {
  ...
  const operation = (async () => {
    ...
    activeSessionContext = normalizeSessionContext({
      serverId,
      profileId,
      sessionKey,
      bindingId,
    ...
    const workspace = workspaceForSession(activeSessionContext, true);
    return panelAnchorTab()
      .then((tab) => attachTabToWorkspace(tab.id, activeSessionContext))
      .catch(() => ({ workspace: publicWorkspace(workspace) }))
      .then(() => ({ ...result, selections: selectionsForSession(activeSessionContext), workspace: publicWorkspace(workspace) }));
...
async function dispatchCommand(name, args, commandSession) {
  if (name === 'browser_status') {
    ...
    return { online: connected, deviceId: settings?.deviceId, deviceName: settings?.deviceName, extensionId: chrome.runtime.id, installationId, activeTab: tab ? publicTab(tab, commandSession) : null, attachedTabIds: [...attachedTabs], ...(await workspaceState(commandSession, { tabScope: 'workspace' })) };
  }
  if (name === 'browser_tabs') {
    const tabs = await chrome.tabs.query({});
    const listed = tabs.filter((tab) => args.includePinned !== false || !tab.pinned).map((tab) => publicTab(tab, commandSession));
    return { workspace: publicWorkspace(workspaceForSession(commandSession)), tabs: args.scope === 'workspace' ? listed.filter((tab) => tab.currentWorkspace) : listed };
  }
  if (name === 'browser_workspace') {
    const workspace = workspaceForSession(commandSession, true);
    if (args.action === 'status') return workspaceState(commandSession, { tabScope: 'workspace' });
    if (args.action === 'attach') return attachTabToWorkspace(Number(args.tabId), commandSession);
    if (args.action === 'detach') return detachTabFromWorkspace(Number(args.tabId), commandSession);
    if (args.action === 'duplicate') {
      const source = await resolveTab(args, commandSession, { requireManaged: true });
      const duplicated = await duplicateWorkspaceTabInBackground(source, commandSession);
      return {
        workspace: publicWorkspace(workspaceForSession(commandSession)),
        tab: publicTab(await chrome.tabs.get(duplicated.id), commandSession),
      };
    }
    if (args.action === 'focus') {
      const tab = await resolveTab(args, commandSession, { requireManaged: true });
      await chrome.tabs.update(tab.id, { active: true });
      await chrome.windows.update(tab.windowId, { focused: true });
      return { focused: true, tabId: tab.id, workspace: publicWorkspace(workspace) };
    }
    if (args.action === 'rename') {
      workspace.title = String(args.title || '').trim();
      await syncWorkspaceGroups(workspace);
      return { renamed: true, workspace: publicWorkspace(workspace) };
    }
    throw errorWithCode('Unsupported workspace action.', 'INVALID_ARGUMENT');
  }
  if (name === 'browser_tab_control') {
    if (args.action === 'new') {
      const workspace = workspaceForSession(commandSession, true);
      const tab = await createBackgroundWorkspaceTab({
        url: args.url ? safeUrl(args.url) : 'about:blank',
        session: commandSession,
      });
      return { tab: publicTab(await chrome.tabs.get(tab.id), commandSession), workspace: publicWorkspace(workspace) };
    }
    const tab = await resolveTab(args, commandSession, { requireManaged: true });
    if (args.action === 'duplicate') {
      const duplicated = await duplicateWorkspaceTabInBackground(tab, commandSession);
      return { tab: publicTab(await chrome.tabs.get(duplicated.id), commandSession), duplicated: true };
    }
  ...
  if (name === 'browser_navigate') {
    const tab = await resolveTab(args, commandSession, { requireManaged: true }); const url = safeUrl(args.url); await chrome.tabs.update(tab.id, { url });
  ...
  if (name === 'browser_snapshot') {
    const tab = await resolveTab(args, commandSession); assertScriptable(tab);
  ...
  if (name === 'browser_inspect') {
    const tab = await resolveTab(args, commandSession);
    let inspected;
  ...
  if (name === 'browser_screenshot') {
    ...
    const imageFormat = args.format === 'jpeg' ? 'jpeg' : 'png';
    const captured = await debuggerCommand(tab.id, 'Page.captureScreenshot', {
      format: imageFormat,
      ...(imageFormat === 'jpeg' ? { quality: Math.min(100, Math.max(20, Number(args.quality) || 85)) } : {}),
      fromSurface: true,
      captureBeyondViewport: false,
    });
    let imageDataUrl = `data:image/${imageFormat};base64,${captured.data}`;
  ...
  if (name === 'browser_click') {
    const tab = await resolveTargetTab(args, commandSession, { requireManaged: true }); assertScriptable(tab);
  ...
  if (name === 'browser_type') {
    const tab = await resolveTargetTab(args, commandSession, { requireManaged: true }); assertScriptable(tab); const selector = resolveTarget(args.target?.selectionId, args.target?.selector, tab.id, commandSession);
  ...
  if (name === 'browser_key') {
    const tab = await resolveTab(args, commandSession, { requireManaged: true }); assertScriptable(tab);
  ...
  if (name === 'browser_evaluate') {
    const tab = await resolveTab(args, commandSession, { requireManaged: true }); const result = await debuggerCommand(tab.id, 'Runtime.evaluate', { expression: String(args.expression), awaitPromise: args.awaitPromise !== false, returnByValue: args.returnByValue !== false, userGesture: true });
  ...
  if (name === 'browser_console') {
    const tab = await resolveTab(args, commandSession); await debuggerCommand(tab.id, 'Runtime.enable'); let entries = [...(consoleByTab.get(tab.id) || [])];
  ...
  if (name === 'browser_network') {
    const tab = await resolveTab(args, commandSession); await debuggerCommand(tab.id, 'Network.enable'); let entries = [...(networkByTab.get(tab.id) || [])];
