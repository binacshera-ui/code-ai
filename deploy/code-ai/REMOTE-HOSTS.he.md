...
אם ntfy מוגדר ב־`.env` של שרת הבקרה, כלי הפריסה מעביר ל־sidecar גם את
כתובת ההתראות, הרשאת הפרסום וה־public origin. כל התראה נושאת את מזהה השרת,
ולחיצה עליה מחזירה את הממשק אוטומטית לשרת ולסשן הנכונים.
...
כאשר ntfy מוגדר בשרת הבקרה, קובץ ה־pairing כולל גם את הגדרות ההתראה. לכן
לאחר שינוי הגדרות ntfy יש לייצר מחדש את קובץ ה־pairing, להעבירו בצורה
מאובטחת ולהפעיל מחדש את ה־personal agent.
