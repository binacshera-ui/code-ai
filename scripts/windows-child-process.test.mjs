...
test('Windows release checks include the tunnel runtime module', async () => {
  const [smokeSource, activationSource] = await Promise.all([
    readSource('scripts/windows-release-smoke.ps1'),
    readSource('scripts/windows-activate-release.ps1'),
  ]);

  assert.match(smokeSource, /personal-computer-tunnel\.mjs/);
  assert.match(smokeSource, /\$agentPath\s+--help/);
  assert.match(activationSource, /personal-computer-tunnel\.mjs/);
});
