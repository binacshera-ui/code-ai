...
$tunnelPath = Join-Path $ReleaseRoot 'scripts\personal-computer-tunnel.mjs'
...
# Resolve every static import used by the Windows supervisor before starting a
# smoke sidecar. This catches incomplete release archives without touching the
# live scheduled task or waiting for its health timeout/rollback path.
& $nodePath $agentPath --help *> $null
if ($LASTEXITCODE -ne 0) {
    throw 'The Windows agent entrypoint or one of its runtime imports is invalid'
}
...
try {
    ...
    $paths = @(
        ...
        "/api/codex/session-conversation-search-mode?profileId=$ProfileId&sessionKey=$sessionKey",
