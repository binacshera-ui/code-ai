    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            await panel.wait_for_function(
                "() => (document.querySelector('#code-ai-frame')?.getAttribute('src') || '').endsWith('/extension-panel')",
                timeout=15_000,
            )
            frame_source = await frame_element.get_attribute("src") or ""
            assert frame_source.endswith("/extension-panel"), {"frameSource": frame_source, "errors": browser_errors}
            ...
            # The extension test drives session binding explicitly below. Stop the
            # embedded app from racing it with its own draft-session sync events.
            await frame_element.evaluate("frame => { frame.src = 'about:blank'; }")
            await panel.wait_for_timeout(150)
            ...
            selection_sync = await panel.evaluate(
                "async context => await chrome.runtime.sendMessage({type: 'SYNC_ACTIVE_SESSION', context})",
                {
                    ...
                    "profileId": profile["id"],
                    "sessionKey": selected_session_key,
                    "sessionTitle": "בדיקת Chromium חיה",
                },
            )
            ...
            workspace_state = panel_state["workspaceState"]
            assert target.url in [tab.get("url") for tab in workspace_state["tabs"] if tab.get("currentWorkspace")]
            target_tab = next(tab for tab in workspace_state["tabs"] if tab.get("url") == target.url)
            assert target_tab["actionAllowed"] is True
            group_id = await worker.evaluate("tabId => chrome.tabs.get(tabId).then(tab => tab.groupId)", target_tab["id"])
            assert group_id >= 0
            group = await worker.evaluate("groupId => chrome.tabGroups.get(groupId)", group_id)
            assert group["title"] == "CODE-AI · בדיקת Chromium חיה", group
            assert len(panel_state["selections"]) == 2

            unmanaged = await context.new_page()
            await unmanaged.goto(f"{BASE_URL}/chat?personalChromeUnmanaged=1", wait_until="domcontentloaded")
            unmanaged_tab_id = await worker.evaluate(
                "url => chrome.tabs.query({}).then(tabs => tabs.find(tab => tab.url === url)?.id)",
                unmanaged.url,
            )
            assert isinstance(unmanaged_tab_id, int)
            await target.bring_to_front()
            ...
            assert selected_context["result"]["count"] == 2, selected_context
            ...
            unmanaged_metadata = next(tab for tab in tabs["result"]["tabs"] if tab.get("id") == unmanaged_tab_id)
            assert unmanaged_metadata["currentWorkspace"] is False
            assert unmanaged_metadata["actionAllowed"] is False

            workspace_status = await asyncio.to_thread(
                request_json,
                "/api/codex/browser-extension/tool-call",
                "POST",
                {"toolName": "browser_workspace", "arguments": {"action": "status"}},
                auth,
            )
            assert unmanaged_tab_id not in [tab.get("id") for tab in workspace_status["result"]["tabs"]]
            assert all(tab.get("currentWorkspace") is True for tab in workspace_status["result"]["tabs"])
            assert workspace_status["result"]["tabCounts"]["visible"] >= len(tabs["result"]["tabs"])
            assert workspace_status["result"]["tabCounts"]["workspace"] == len(workspace_status["result"]["tabs"])

            unmanaged_snapshot = await asyncio.to_thread(
                request_json,
                "/api/codex/browser-extension/tool-call",
                "POST",
                {"toolName": "browser_snapshot", "arguments": {"tabId": unmanaged_tab_id, "maxChars": 4000}},
                auth,
            )
            assert unmanaged_snapshot["result"]["url"].endswith("personalChromeUnmanaged=1")

            blocked_navigation = await asyncio.to_thread(
                request_json,
                "/api/codex/browser-extension/tool-call",
                "POST",
                {"toolName": "browser_navigate", "arguments": {"tabId": unmanaged_tab_id, "url": f"{BASE_URL}/blocked"}},
                auth,
                400,
            )
            assert blocked_navigation["error"]["code"] == "TAB_NOT_ASSIGNED"

            attached = await asyncio.to_thread(
                request_json,
                "/api/codex/browser-extension/tool-call",
                "POST",
                {"toolName": "browser_workspace", "arguments": {"action": "attach", "tabId": unmanaged_tab_id}},
                auth,
            )
            assert unmanaged_tab_id in attached["result"]["workspace"]["tabIds"]
            unmanaged_group_id = await worker.evaluate("tabId => chrome.tabs.get(tabId).then(tab => tab.groupId)", unmanaged_tab_id)
            assert unmanaged_group_id >= 0

            duplicated = await asyncio.to_thread(
                request_json,
                "/api/codex/browser-extension/tool-call",
                "POST",
                {"toolName": "browser_workspace", "arguments": {"action": "duplicate", "tabId": unmanaged_tab_id}},
                auth,
            )
            duplicated_tab_id = duplicated["result"]["tab"]["id"]
            assert duplicated_tab_id in duplicated["result"]["workspace"]["tabIds"]
            ...
            screenshot = await asyncio.to_thread(
                ...
                "/api/codex/browser-extension/tool-call",
                "POST",
                {"toolName": "browser_screenshot", "arguments": {"tabId": target_tab["id"], "format": "jpeg", "quality": 60}},
                auth,
            )
            assert screenshot["result"]["imageDataUrl"].startswith("data:image/jpeg;base64,")
            assert screenshot["result"]["bytes"] > 100

            user_foreground = await context.new_page()
            await user_foreground.goto(f"{BASE_URL}/chat?personalChromeUserForeground=1", wait_until="domcontentloaded")
            await user_foreground.bring_to_front()
            foreground_tab_id = await worker.evaluate(
                "chrome.tabs.query({active: true, currentWindow: true}).then(tabs => tabs[0]?.id)"
            )
            assert isinstance(foreground_tab_id, int)

            async def assert_user_focus_preserved():
                active_tab_id = await worker.evaluate(
                    "chrome.tabs.query({active: true, currentWindow: true}).then(tabs => tabs[0]?.id)"
                )
                assert active_tab_id == foreground_tab_id, {
                    "expectedForegroundTabId": foreground_tab_id,
                    "actualActiveTabId": active_tab_id,
                }

            background_navigation = await asyncio.to_thread(
                request_json,
                "/api/codex/browser-extension/tool-call",
                "POST",
                {
                    ...
                    "arguments": {
                        "tabId": target_tab["id"],
                        "url": f"{BASE_URL}/chat?personalChromeBackgroundNavigation=1",
                        "waitUntil": "domcontentloaded",
                    },
                },
                auth,
            )
            assert background_navigation["result"]["tabId"] == target_tab["id"]
            await assert_user_focus_preserved()

            await asyncio.to_thread(
                request_json,
                "/api/codex/browser-extension/tool-call",
                "POST",
                {"toolName": "browser_scroll", "arguments": {"tabId": target_tab["id"], "deltaY": 320}},
                auth,
            )
            await assert_user_focus_preserved()
            ...
            await asyncio.to_thread(
                ...
                "/api/codex/browser-extension/tool-call",
                "POST",
                {"toolName": "browser_screenshot", "arguments": {"tabId": target_tab["id"], "format": "jpeg", "quality": 50}},
                auth,
            )
            await assert_user_focus_preserved()

            background_new_tab = await asyncio.to_thread(
                request_json,
                "/api/codex/browser-extension/tool-call",
                "POST",
                {
                    "toolName": "browser_tab_control",
                    "arguments": {"action": "new", "url": f"{BASE_URL}/chat?personalChromeBackgroundNew=1"},
                },
                auth,
            )
            assert background_new_tab["result"]["tab"]["currentWorkspace"] is True
            assert background_new_tab["result"]["tab"]["active"] is False
            await assert_user_focus_preserved()

            background_duplicate = await asyncio.to_thread(
                request_json,
                "/api/codex/browser-extension/tool-call",
                "POST",
                {"toolName": "browser_tab_control", "arguments": {"action": "duplicate", "tabId": target_tab["id"]}},
                auth,
            )
            assert background_duplicate["result"]["tab"]["currentWorkspace"] is True
            assert background_duplicate["result"]["tab"]["active"] is False
            await assert_user_focus_preserved()
            assert not any(
            ...
            print(json.dumps({
                ...
                "deviceId": device_id,
                "autoRecovered": True,
                "tools": ["browser_tabs", "browser_workspace", "browser_snapshot", "browser_screenshot", "browser_selection_context"],
                "picker": [picker_result["selection"]["kind"], region_result["selection"]["kind"]],
                "selectionContextVersion": 2,
                "workspaceGroupId": group_id,
                "blockedUnmanagedMutation": True,
                "duplicatedManagedTabId": duplicated_tab_id,
                "backgroundAutomationPreservedUserFocus": True,
            }))
        finally:
            if binding_id:
                await asyncio.to_thread(
